...
describe Api::V1::User::TaskProject::Contract::CreateUpdate, type: :contract do
  subject(:contract) do
    contract = described_class.new(TaskProject.new)
  ...
  describe 'FAILURE' do
    context 'when area not exist' do
      let(:params) { default_params.merge(task_area_id: SecureRandom.uuid) }
      let(:expected_errors) do
        {
          task_area_id: [I18n.t('errors.area_existence?')]
        }
      end
    ...
    context 'when area not belongs to user' do
      ...
      let(:params) { default_params.merge(task_area_id: area.id) }
