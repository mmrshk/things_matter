...
describe 'mutation userTaskProjectDelete', type: :request do
  ...
  context 'when success' do
    it 'returns updated project of current_user' do
      authorized_graphql_post(
        query: user_task_project_delete_mutation,
        variables: variables,
        auth_token: token
      )

      expect(response).to match_schema(UserTaskProject::DeleteSchema)
