...
describe 'mutation userNoteTagDelete', type: :request do
  ...
  let(:variables) { { input: { id: note_tag.id } } }

  context 'when success' do
    it 'returns updated note tag of current_user' do
      authorized_graphql_post(
        query: user_note_tag_delete_mutation,
        variables: variables,
        auth_token: token
      )

      expect(response).to match_schema(UserNoteTag::DeleteSchema)
      expect(response.status).to be(200)
    end
  end

  context 'when failed' do
    context 'when note tag not found' do
